...
// related source code.
   ...
   #define WD_PROD_NAME "WinDriver Lite"
#else
...
#define WD_VER_STR  WD_PROD_NAME " v" WD_VERSION_STR \
    " Jungo (c) 1997 - 2013 Build Date: " __DATE__ \
...
enum
{
    ...
    LICENSE2_OS_LIMITED = 0x00000200 /* Limit WD to demo license on future OS versions */
};
...
typedef enum
{
    WD_ITEM_DO_NOT_MAP_KERNEL = 1, /* Skip the mapping of physical memory to
                                      the kernel address space */
    WD_ITEM_ALLOW_CACHE = 2,       /* Map physical memory as cached; applicable
                                      only to host RAM, not to local memory on
                                      the card */
    WD_ITEM_CE_MAP_VIRTUAL = 3     /* For Windows CE only -- perform kernel
                                      mapping of physical memory to non-static
                                      virtual system addresses, as opposed to
                                      the default static kernel address mapping.
                                      NOTE: Do not set this flag for items that
                                      need to be accessed in kernel mode --
                                      namely, items that will be used to create
                                      interrupt transfer commands or items that
                                      will be used from a Kernel PlugIn driver.
                                    */
} WD_ITEM_OPTIONS;
...
typedef struct
{
    ...
    DWORD dwOptions;    /* Bitmask of WD_ITEM_OPTIONS flags */
    union
    {
        struct
        { /* ITEM_MEMORY */
            ...
            KPTR dwTransAddr;           /* Kernel-mode mapping of the physical
                                           base address, to be used for
                                           transfer commands; returned by
                                           WD_CardRegister()
                                         */
            UPTR dwUserDirectAddr;      /* User-mode mapping of the physical
                                           base address, for direct user
                                           read/write transfers; returned by
                                           WD_CardRegister()
                                         */
            DWORD dwCpuPhysicalAddr;    /* CPU physical address; returned by
                                           WD_CardRegister()
                                         */
        ...
        struct
        { /* ITEM_IO */
            KPTR dwAddr;          /* Beginning of I/O address */
...
#define IOCTL_WD_CARD_REGISTER            WD_CTL_CODE(0x9db)
...
#define IOCTL_WD_EVENT_REGISTER           WD_CTL_CODE(0x9dc)
