...
// BOOL and HANDLE are brought in here from pixie500e_lib.h for it to build on Linux.
/*
#ifndef BOOL
...
WDC_DEVICE_HANDLE PIXIE500E_DeviceOpen(const WD_PCI_CARD_INFO *pDeviceInfo)
{
	...
	dwStatus = WDC_PciDeviceOpen(&hDev, pDeviceInfo, pDevCtx/*, NULL, NULL, NULL*/);//by Hongyi Wu
...
DWORD PIXIE500E_IntEnable(WDC_DEVICE_HANDLE hDev, PIXIE500E_INT_HANDLER funcIntHandler)
{
	...
	*      for each interrupt that is received */
	...
	pDevCtx->pIntTransCmds[numOfTransCmd].pPort = pAddrDesc->pAddr + 0x814;// by Hongyi Wu
	...
	if((intMask & 0xF0) != 0) { // if External Interrupt enabled
		...
		*/
		...
		pDevCtx->pIntTransCmds[numOfTransCmd].pPort = pAddrDesc->pAddr + 0x14; // 0x14 is VDMA_EVENT_ENx              //by Hongyi Wu
...
BOOL PIXIE500E_GetAddrSpaceInfo(WDC_DEVICE_HANDLE hDev, PIXIE500E_ADDR_SPACE_INFO *pAddrSpaceInfo)
{
	...
	if (WDC_AddrSpaceIsActive(pDev, dwAddrSpace))
	{
		...
		DWORD dwAddr = fIsMemory ? pItem->I.Mem.pPhysicalAddr : (DWORD)pItem->I.IO.pAddr;//by Hongyi Wu

		snprintf(pAddrSpaceInfo->sDesc, MAX_DESC - 1, "0x%0*lX - 0x%0*lX (0x%lx bytes)",
			 (int)WDC_SIZE_32 * 2, dwAddr,
			 (int)WDC_SIZE_32 * 2, dwAddr + pAddrDesc->qwBytes - 1,
			 pAddrDesc->qwBytes); //by Hongyi Wu
	}
	else
...
#define DEBUG_PRINT 0
	...
	char ErrMSG[256] = {0};
	DWORD dwStatus = WD_WINDRIVER_STATUS_ERROR;
	UINT32 len = N_P4E_BYTES; // initialize to default P4e
	UINT8  value;
	UINT32 i, j, k, word_counter = 0, doneInt = 0;
	UINT32 dwData, svData;
	INT16 fcl_irq, fcl_ctrl;
	UINT8 *pu8Ptr = P500e_FPGA_Configuration;		// initialize to default P4e
	UINT8 tmpU8Ptr[4]; // temp buffer for bit-reversed configuration chunks to go in FCL_FIFO_DATA

	if(type == MODULETYPE_P4e)
	{
		*pu8Ptr = P4e_FPGA_Configuration;
		len = N_P4E_BYTES;
	}
	else if(type == MODULETYPE_P500e)
	{
		*pu8Ptr = P500e_FPGA_Configuration;
		len = N_P500E_BYTES;
	}
	else if(type == MODULETYPE_P32)
	{
			*pu8Ptr = P32_FPGA_Configuration;
			len = N_P32_BYTES;
			sprintf(ErrMSG, "*DEBUG* (PIXIE500E_ProgramFPGA(): P32 configuration");
			Pixie_Print_MSG(ErrMSG,PrintDebugMsg_Boot);
			//type=12; // KS DEBUG: to force re-configuration
	}
	else
	{
		sprintf(ErrMSG, "*DEBUG* (PIXIE500E_ProgramFPGA(): Unknown Module Type %X", type);
		Pixie_Print_MSG(ErrMSG,1);
		return (-3);//unknown module type;
	}

/* does not seem to help. PLL reset from Igor before boot _sometimes_ works
	...
	sprintf(ErrMSG, "*DEBUG* (PIXIE500E_ProgramFPGA): read CLK CSR %x.",dwData);
	Pixie_Print_MSG(ErrMSG,1);
	svData = dwData;
	dwData = dwData | (U32)(pow(2.0,27));												// set reset bit
	sprintf(ErrMSG, "*DEBUG* (PIXIE500E_ProgramFPGA): write CLK CSR %x.",dwData);
	Pixie_Print_MSG(ErrMSG,1);
...
*/
	...
	// TODO: (KS) We do not check the FRCS header information to find the start
	// of the bitsream data, assuming a regular bistream file.

	// GenDialog had the RSTOUT assert/LB disable/LB enable/RSTOUT de-assert.
	// It seems that we don't really have to do it (but still, if BAR0 problems are due to
	// relative phases of clocks, we might power up in wrong phase, it breaks everything.
	// (this is of FW r19627)

	// Assert RSTOUT
	dwStatus = PIXIE500E_ReadWriteReg(hDev, PCI_SYS_CFG_SYSTEM, WDC_READ, &i, FALSE);
	if( (dwStatus==WD_WINDRIVER_STATUS_ERROR) || (i==0xFFFFFFFF) ) return REGIO_ERR;		// fatal error causing system crash
	i &= ~(0xC000);
	...
	if( (dwStatus==WD_WINDRIVER_STATUS_ERROR) || (fcl_ctrl==0xFFFFFFFF) ) return REGIO_ERR;		// fatal error causing system crash

	if( fcl_ctrl != 0x40 ) {
		sprintf(ErrMSG, "*DEBUG* (PIXIE500E_ProgramFPGA(): Error in FCL_CTL=0x%04X", fcl_ctrl);
		Pixie_Print_MSG(ErrMSG,1);
	...
	*/
	...
	while(len > 0) { // load configuration
		...
		dwStatus = PIXIE500E_ReadWriteReg(hDev, FCL_IRQ, WDC_READ, &fcl_irq, FALSE);
#if (DEBUG_PRINT)
		sprintf(ErrMSG, "*DEBUG* (PIXIE500E_ProgramFPGA(): FCL_IRQ = 0x%04X len=%ld", fcl_irq, len);
		Pixie_Print_MSG(ErrMSG,1);
#endif
		if((fcl_irq & 0x8) && (word_counter > 0))
		{
			doneInt = 1;
#if (DEBUG_PRINT)
			printf("*DEBUG* (PIXIE500E_ProgramFPGA(): CONFIG_DONE detected!");
#endif
		}
		else if((fcl_irq & 0x4) && !doneInt)
		{
			sprintf(ErrMSG, "*DEBUG* (PIXIE500E_ProgramFPGA(): FCL reset on write error detected!");
			Pixie_Print_MSG(ErrMSG,1);
	...
	while(!doneInt) {
		...
		if(fcl_irq & 0x8)
			...
			sprintf(ErrMSG, "*DEBUG* (PIXIE500E_ProgramFPGA(): ERROR 6 detected!");
			Pixie_Print_MSG(ErrMSG,1);
			return(-6);// WD_WINDRIVER_STATUS_ERROR;
		}

		if(i++ > PIXIE500E_FCL_TIME_OUT){
			sprintf(ErrMSG, "*DEBUG* (PIXIE500E_ProgramFPGA(): TIMEOUT on waiting DONE interrupt! FCL_IRQ=0x%04X dwStatus=%ld", fcl_irq, dwStatus);
			Pixie_Print_MSG(ErrMSG,1);                 
...
#define DEBUG_PRINT 0
	...
	sprintf(ErrMSG, "*INFO* (PIXIE4E_ProgramFPGA(): BEGIN");
	Pixie_Print_MSG(ErrMSG,1);
	...
	switch (type) {
		case (UINT32)MODULETYPE_P500e:
			pu8Ptr = P500e_FPGA_Configuration;
			len = N_P500E_BYTES;
			sprintf(ErrMSG, "*DEBUG* (PIXIE4E_ProgramFPGA(): P500e configuration");
			Pixie_Print_MSG(ErrMSG,PrintDebugMsg_Boot);
			break;
		case (UINT32)MODULETYPE_P4e_16_125:
			pu8Ptr = P4e_FPGA_Configuration;
			len = N_P4E_BYTES;
			sprintf(ErrMSG, "*DEBUG* (PIXIE4E_ProgramFPGA(): P4e_16_125 configuration");
			Pixie_Print_MSG(ErrMSG,PrintDebugMsg_Boot);
			break;
		case (UINT32)MODULETYPE_P4e_14_500:
			pu8Ptr = P4e14500_FPGA_Configuration;
			len = N_P4E_BYTES;
			sprintf(ErrMSG, "*DEBUG* (PIXIE4E_ProgramFPGA(): P4e_14_500 configuration");
			Pixie_Print_MSG(ErrMSG,PrintDebugMsg_Boot);
			break;
		case (UINT32)MODULETYPE_P32:
			pu8Ptr = P32_FPGA_Configuration;
			len = N_P32_BYTES;
			sprintf(ErrMSG, "*DEBUG* (PIXIE4E_ProgramFPGA(): P32 configuration");
			Pixie_Print_MSG(ErrMSG,PrintDebugMsg_Boot);
			//type=12; // KS DEBUG: to force re-configuration
			break;
		case 12: // Force FPGA re-configuration (test)
			...
			pu8Ptr = P4e_FPGA_Configuration;
			len = N_P4E_BYTES;
			sprintf(ErrMSG, "*DEBUG* (PIXIE4E_ProgramFPGA(): P4e_16_125 configuration, force re-configuration (type 12)");
			Pixie_Print_MSG(ErrMSG,1);
			break;
		case 13: // Force FPGA re-configuration (test)
			...
			pu8Ptr = P4e14500_FPGA_Configuration;
			len = N_P4E_BYTES;
			sprintf(ErrMSG, "*DEBUG* (PIXIE4E_ProgramFPGA(): P4e_14_500 configuration, force re-configuration (type 13)");
			Pixie_Print_MSG(ErrMSG,1);
			break;
		default:
			sprintf(ErrMSG, "*ERROR* (PIXIE4E_ProgramFPGA(): Unknown Module Type %X", type);
			Pixie_Print_MSG(ErrMSG,1);
			return (-3);//unknown module type;
  ...
  //tricks to allow FPGA reboot ...does not seem to help. PLL reset from Igor before boot _sometimes_ works
	...
	if( (dwStatus==WD_WINDRIVER_STATUS_ERROR) || (dwData==0xFFFFFFFF) ) {
		sprintf(ErrMSG, "*ERROR* (PIXIE4E_ProgramFPGA(): Problem reading CLK_SCR=0x%08X", dwData);
		Pixie_Print_MSG(ErrMSG,1);
		return REGIO_ERR;		// fatal error causing system crash
	}

	sprintf(ErrMSG, "*DEBUG* (PIXIE4E_ProgramFPGA): read CLK CSR 0x%08X.",dwData);
	Pixie_Print_MSG(ErrMSG,PrintDebugMsg_Boot);
...
//	Pixie_Print_MSG(ErrMSG,1);    
	if((fcl_status & 0x8) == 0x8){ // if already configured
		if(type==12 || type==13) {
			sprintf(ErrMSG, "*WARNING* (PIXIE4E_ProgramFPGA(): FPGA already programmed, BUT NOW REPROGRAMMING (debug mode)");
			Pixie_Print_MSG(ErrMSG,1); 
		} 
		else {
			sprintf(ErrMSG, "*WARNING* (PIXIE4E_ProgramFPGA(): FPGA already programmed, skipping. Power cycle chassis to reprogram");
			Pixie_Print_MSG(ErrMSG,1);  
	...
	// turn off DMA and other read processes, and flush
//	sprintf(ErrMSG, "*Debug* (PIXIE4E_ProgramFPGA(): turning off DMA and other read processes, and flush");
//	Pixie_Print_MSG(ErrMSG,PrintDebugMsg_Boot); 
//	i = 0x00000000;	// bit0-3: ldm/pdm/csr enable. bit4: if 1 flush dma process. 
//	dwStatus = PIXIE500E_ReadWriteReg(hDev, DMA_CFG, WDC_WRITE, &i, FALSE);
	...
	if( fcl_ctrl != 0x40 ) {
		sprintf(ErrMSG, "*ERROR* (PIXIE4E_ProgramFPGA(): Error in FCL_CTL=0x%04X", fcl_ctrl);
		Pixie_Print_MSG(ErrMSG,1);
		return (-4);//WD_WINDRIVER_STATUS_ERROR;
	}
	...
	*/
	...
	while(len > 0) { // load configuration
		...
		{
			sprintf(ErrMSG, "*ERROR* (PIXIE4E_ProgramFPGA(): FCL reset on write error detected!");
			Pixie_Print_MSG(ErrMSG,1);
			return(-5);// WD_WINDRIVER_STATUS_ERROR;
		}
	...
	while(!doneInt) {
		...
		if(fcl_irq & 0x8)
			...
			sprintf(ErrMSG, "*ERROR* (PIXIE4E_ProgramFPGA(): ERROR 6 detected!");
			Pixie_Print_MSG(ErrMSG,1);
			return(-6);// WD_WINDRIVER_STATUS_ERROR;
		}

		if(i++ > PIXIE500E_FCL_TIME_OUT){
			sprintf(ErrMSG, "*ERROR* (PIXIE4E_ProgramFPGA(): TIMEOUT on waiting DONE interrupt! FCL_IRQ=0x%04X dwStatus=%ld", fcl_irq, dwStatus);
			Pixie_Print_MSG(ErrMSG,1);                 
			return(-7);// WD_WINDRIVER_STATUS_ERROR;
		}
	} // while not done
...
//	i = 0x00000007;	// bit0-3: ldm/pdm/csr enable. bit4: if 1 flush dma process. 
//	dwStatus = PIXIE500E_ReadWriteReg(hDev, DMA_CFG, WDC_WRITE, &i, FALSE);
//	Pixie_Sleep(150);
	...
	sprintf(ErrMSG, "*DEBUG*(PIXIE4E_ProgramFPGA): assume BAR0 problems, doing PLL reset...");
	Pixie_Print_MSG(ErrMSG,PrintDebugMsg_Boot);
	PIXIE500E_LBClkReset(hDev);
	sprintf(ErrMSG, "*DEBUG*(PIXIE4E_ProgramFPGA): got BAR0 back?");
	Pixie_Print_MSG(ErrMSG,PrintDebugMsg_Boot);
	//}
	...
	if ((dwData & 0xF0000000) != 0xE0000000) {
		sprintf(ErrMSG, "*ERROR*(PIXIE4E_ProgramFPGA): Problem with PLL lock, CLK_CSR=0x%08X. Please reboot", dwData);
		Pixie_Print_MSG(ErrMSG,1);
		return(-4);
	}
	sprintf(ErrMSG, "*DEBUG*(PIXIE4E_ProgramFPGA): PLL is locked, CLK_CSR=0x%08X", dwData);
	Pixie_Print_MSG(ErrMSG,PrintDebugMsg_Boot);

	// When forcing re-configuration, losing BAR0 here (CID changed to 1 on re-configuration)

	// Read from BAR0
	sprintf(ErrMSG, "*DEBUG*(PIXIE4E_ProgramFPGA): checking BAR0:");
	Pixie_Print_MSG(ErrMSG,PrintDebugMsg_Boot);

	dwStatus = PIXIE500E_ReadWriteReg(hDev, PCIE_BAR0_LOW, WDC_READ, &i, FALSE);
	sprintf(ErrMSG, "*DEBUG*(PIXIE4E_ProgramFPGA): (BAR4) PCIE_BAR0_LOW = 0x%08X ", i);
	Pixie_Print_MSG(ErrMSG,PrintDebugMsg_Boot);

	dwStatus = PIXIE500E_ReadWriteReg(hDev, PL_WR_BRG_RD_THRES, WDC_READ, &i, FALSE);
	sprintf(ErrMSG, "*DEBUG*(PIXIE4E_ProgramFPGA): PL_WR_BRG_RD_THRES = 0x%08X ", i);
	Pixie_Print_MSG(ErrMSG,PrintDebugMsg_Boot);

	// check if SDRAM initialized
	dwStatus = PIXIE500E_ReadWriteReg(hDev, APP_SDRAM_STATUS, WDC_READ, &i, FALSE);
	sprintf(ErrMSG, "*DEBUG*(PIXIE4E_ProgramFPGA): APP_SDRAM_STATUS = 0x%08X ", i);
	Pixie_Print_MSG(ErrMSG,PrintDebugMsg_Boot);
	...
	if(dwStatus !=0) {
		sprintf(ErrMSG, "*ERROR* (PIXIE4E_ProgramFPGA(): PIXIE4E_FPGApostboot_IO failed");
		Pixie_Print_MSG(ErrMSG,1);  
		return (dwStatus);
	}

	sprintf(ErrMSG, "*INFO* (PIXIE4E_ProgramFPGA(): END");
	Pixie_Print_MSG(ErrMSG,1);
	return 0;
}
...
//
	...
	if ( (i & 0x4) >> 2 == 0) { // if SDRAM INIT_DONE is not set, need to calibrate
		...
		sprintf(ErrMSG, "*INFO*(PIXIE4E_FPGApostboot_IO): SDRAM INIT ok ");
		Pixie_Print_MSG(ErrMSG,PrintDebugMsg_Boot);
	}
...
{
	UINT32 originalSettings, postRefClk, dwStatus, i, tmp,modifiedSettings;
	...
	Pixie_Print_MSG(ErrMSG, PrintDebugMsg_Boot);
	modifiedSettings = originalSettings | 0x8080000;
	sprintf(ErrMSG, "*DEBUG* (LBClkReset): writing 0x%08X", modifiedSettings);
	Pixie_Print_MSG(ErrMSG, PrintDebugMsg_Boot);
	dwStatus = PIXIE500E_ReadWriteReg(hDev, CLK_CSR, WDC_WRITE, &modifiedSettings, FALSE);
	dwStatus = PIXIE500E_ReadWriteReg(hDev, CLK_CSR, WDC_READ, &modifiedSettings, FALSE);		//Read back to ensure write completion	
	//originalSettings &= ~0x8080000;
	sprintf(ErrMSG, "*DEBUG* (LBClkReset): writing 0x%08X, then sleep", originalSettings);
	Pixie_Print_MSG(ErrMSG, PrintDebugMsg_Boot);
	dwStatus = PIXIE500E_ReadWriteReg(hDev, CLK_CSR, WDC_WRITE, &originalSettings, FALSE);
	Pixie_Sleep(500);
	dwStatus = PIXIE500E_ReadWriteReg(hDev, CLK_CSR, WDC_READ, &modifiedSettings, FALSE);	//Read back to ensure write completion
	sprintf(ErrMSG, "*DEBUG* (LBClkReset): reading 0x%08X", modifiedSettings);
	Pixie_Print_MSG(ErrMSG, PrintDebugMsg_Boot);
...
DWORD VDMADriver_Go(WDC_DEVICE_HANDLE hDev)
{
	...
	sprintf(ErrMSG, "*DEBUG* (VDMADriver_Go): before starting, CSR=0x%08X", val);
	Pixie_Print_MSG(ErrMSG,PrintDebugMsg_daq);
	do  {
		dwStatus = PIXIE500E_ReadWriteReg(hDev, VDMA_CSRx, WDC_WRITE, &cmd, FALSE);		//write 1 to bit 0. writing 0 to bit 1 has no effect
//		Pixie_Sleep(2);	// adding this sleep makes DMA not work at all???
		dwStatus = PIXIE500E_ReadWriteReg(hDev, VDMA_CSRx, WDC_READ, &val, FALSE);
		timeout++;
...
DWORD VDMADriver_Halt(WDC_DEVICE_HANDLE hDev)
{
	...
	DWORD dwStatus = WD_WINDRIVER_STATUS_ERROR;
	dwStatus = PIXIE500E_ReadWriteReg(hDev, VDMA_CSRx, WDC_READ, &val, FALSE);
	sprintf(ErrMSG, "*DEBUG* (VDMADriver_Halt): before halting, CSR=0x%08X", val);
	Pixie_Print_MSG(ErrMSG,PrintDebugMsg_daq);
	if(val!=0)	// only halt if it's not already halted
	{
		do  {
			//dwStatus=WriteMemoryAddrSpace(BAR0, VDMA_CSRx, &cmd, REG_32);
			dwStatus = PIXIE500E_ReadWriteReg(hDev, VDMA_CSRx, WDC_WRITE, &cmd, FALSE);		// write 1 to bit 1. writing 0 to bit 0 has no effect
			dwStatus = PIXIE500E_ReadWriteReg(hDev, VDMA_CSRx, WDC_READ, &val, FALSE);		// in FW, should clear both bit 0 and 1
			timeout++;
			sprintf(ErrMSG, "*DEBUG* (VDMADriver_Halt): CSR=0x%08X", val);
			Pixie_Print_MSG(ErrMSG,PrintDebugMsg_daq);
		} while ( (timeout<10) & (val !=0)) ; 
		if(timeout>=10) {
			sprintf(ErrMSG, "*ERROR* (VDMADriver_Halt): Stopping DMA failed");
			Pixie_Print_MSG(ErrMSG,1);
			return WD_WINDRIVER_STATUS_ERROR;
		}
	}
...
UINT32 PIXIE500E_ProgramDSP(WDC_DEVICE_HANDLE hDev)
{
	...
	/**********************************************************************************/
/*
	// experimental simplified booting.
	// we download all data from the file anyway and add no padding words for non-even 48bit transfers (it's all in the ldr file already). 
	// So no need to parse it for tags, just download it straight
	...
	while (i < dwLdrFileLengthBytes/WDC_SIZE_32) {
		...
		//		Pixie_Sleep(1);				// by commenting this out, we rely here on only very short "zero N words" tags that the DSP can finish in the 30k wait cycles. Else it's very slow!
				if((i&0x00FF) == 0) {
					sprintf(ErrMSG, "*INFO* (PIXIE500E_ProgramDSP): i = %d of %d total", i,dwLdrFileLengthBytes/WDC_SIZE_32);
					Pixie_Print_MSG(ErrMSG,PrintDebugMsg_Boot);
					pauseAfterZero = FALSE;
				}
		//	}
	...
	// unused code. keep around for debugging

	*/


	while (i < dwLdrFileLengthBytes/WDC_SIZE_32) {
		// Assume that data is (in 32bit format as read in):
		// 0x0000 00TT (tag)
		// 0x0000 LLLL (length)
		// 0xAAAA AAAA (address)
		wLdrTag = (UINT16)(buffer[headerIndex] & 0x000000FF);
		wLdrCount = (UINT16)((buffer[headerIndex+1] & 0x0000FFFF));
		dwDataTmp = buffer[headerIndex+2];
		sprintf(ErrMSG, "*INFO* (PIXIE500E_ProgramDSP): tag, count, addr: 0x %04x %04x %08x", wLdrTag,wLdrCount,dwDataTmp  );
		Pixie_Print_MSG(ErrMSG,0);
