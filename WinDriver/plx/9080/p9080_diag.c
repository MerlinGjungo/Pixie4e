...
*  File: p9080_diag.c
*
*  Sample user-mode diagnostics application for accessing PLX 9080
...
#define P9080_DEFAULT_VENDOR_ID 0x10b5 /* Vendor ID */
#define P9080_DEFAULT_DEVICE_ID 0x9080 /* Device ID */
...
int main(void)
{
    ...
    printf("\n");
    printf("PLX 9080 diagnostic utility.\n");
    ...
    {
        PLX_DIAG_ERR("p9080_diag: Failed to initialize the PLX library: %s",
            PLX_GetLastErr());
        return dwStatus;
    }

    /* Find and open a PLX 9080 device (by default ID) */
    if (P9080_DEFAULT_VENDOR_ID)
    {
        hDev = PLX_DIAG_DeviceFindAndOpen(P9080_DEFAULT_VENDOR_ID,
            P9080_DEFAULT_DEVICE_ID, IS_MASTER);
        if (!hDev)
        {
            PLX_DIAG_ERR("p9080_diag: Failed locating and opening a handle to "
                "device (VID 0x%x DID 0x%x)\n", P9080_DEFAULT_VENDOR_ID,
                P9080_DEFAULT_DEVICE_ID);
        }
    }
    ...
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        PLX_DIAG_ERR("p9080_diag: Failed to uninit the PLX library: %s",
            PLX_GetLastErr());
    }
    ...
    do
    {
        ...
        case MENU_MAIN_FIND_AND_OPEN: /* Find and open a PLX device */
            ...
            *phDev = PLX_DIAG_DeviceFindAndOpen(0, 0, IS_MASTER);
            if (!*phDev)
            {
                PLX_DIAG_ERR("p9080_diag: Failed locating and opening a handle "
        ...
        case MENU_MAIN_EEPROM:
            ...
            PLX_DIAG_MenuEEPROM(*phDev, BIT5);
            break;
    ...
    PLX_INT_RESULT *pIntResult)
{
    ...
    printf("Got DMA interrupt, counter %ld, INTCSR 0x%X, DMACSR 0x%X\n",
