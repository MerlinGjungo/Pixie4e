...
/* TODO: When using a registered WinDriver version, replace the license string
         ...
         the driver name below with your driver name */
...
DWORD PLX_DeviceOpenBySlot(WDC_DEVICE_HANDLE *pDeviceHandle,
    const WD_PCI_SLOT *pSlot, BOOL fIsMaster)
{
    DWORD dwStatus;
    ...
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        ErrLog("PLX_DeviceOpenBySlot: Failed retrieving the device's resources "
            "information.\nError 0x%lx - %s\n", dwStatus, Stat2Str(dwStatus));
        return dwStatus;
    }
    ...
    if (!pDevCtx)
    {
        ErrLog("PLX_DeviceOpenBySlot: Failed allocating memory for PLX device "
            "context\n");
    ...
    DWORD dwDeviceID, DWORD nCardNum, BOOL fIsMaster)
{
    ...
    {
        ErrLog("PLX_DeviceOpen: No matching PLX device was found for search "
            "criteria (Vendor ID 0x%lX, Device ID 0x%lX, Card Nmuber %ld)\n",
    ...
    PLX_DMA_HANDLE hDma)
{
    ...
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        ErrLog("IntEnableDma: Failed enabling interrupts (vid 0x%lX did 0x%lX, "
...
static DWORD IntEnable(PWDC_DEVICE pDev, PLX_INT_HANDLER funcDiagIntHandler)
{
    ...
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        ErrLog("IntEnable: Failed enabling interrupts (vid 0x%lX did 0x%lX, "
            "handle 0x%p).\nError 0x%lx - %s\n", pDevCtx->id.dwVendorId,
            pDevCtx->id.dwDeviceId, pDev, dwStatus, Stat2Str(dwStatus));
        goto Error;
    }
    ...
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        ErrLog("IntEnable: Faild writing to the INTCSR register to physically "
            "enable the interrupts on the board (vid 0x%lX did 0x%lX, "
            "handle 0x%p)\nError 0x%lx - %s\n", pDevCtx->id.dwVendorId,
            pDevCtx->id.dwDeviceId, pDev, dwStatus, Stat2Str(dwStatus));
        goto Error;
    }
    ...
    PLX_DMA_CHANNEL dmaChannel, PLX_DMA_HANDLE *ppDmaHandle)
{
    ...
    else /* DMA of more then one page ==> chain DMA */
    {
        ...
        dwStatus = WDC_DMAContigBufLock(hDev, (void *)&pList, DMA_TO_DEVICE,
            pPLXDma->pDma->dwPages * sizeof(DMA_LIST) + 0x10,
            &pPLXDma->pDmaList);
        if (WD_STATUS_SUCCESS != dwStatus)
        {
            ...
            dwPageNumber++)
        {
            pList[dwPageNumber].u32PADR =
                (UINT32)pPLXDma->pDma->Page[dwPageNumber].pPhysicalAddr;
            pList[dwPageNumber].u32LADR =
                u32LocalAddr + (fAutoinc ? u32MemoryCopied : 0);
            pList[dwPageNumber].u32SIZ =
                (UINT32)pPLXDma->pDma->Page[dwPageNumber].dwBytes;
    ...
    PLX_ADDR_SPACE_INFO *pAddrSpaceInfo)
{
    ...
    if (!pAddrSpaceInfo)
    {
        ErrLog("PLX_GetAddrSpaceInfo: Error - NULL address space information "
            "pointer\n");
        return FALSE;
    }
#endif
    ...
    {
        ErrLog("PLX_GetAddrSpaceInfo: Error - Address space %ld is out of "
    ...
    if (WDC_AddrSpaceIsActive(pDev, dwAddrSpace))
    {
        ...
        snprintf(pAddrSpaceInfo->sDesc, MAX_DESC - 1,
            "0x%0*"PRI64"X - 0x%0*"PRI64"X (%"PRI64"d bytes)",
    ...
    UINT32 *pu32Data)
{
    ...
    if (dwOffset % 4)
    {
        ErrLog("PLX_EEPROM_VPD_Read32: Error - offset (0x%lX) is not a "
    ...
    UINT32 u32Data)
{
    ...
    if (dwOffset % 4)
    {
        ErrLog("PLX_EEPROM_VPD_Write32: Error - offset (0x%lX) is not a "
            "multiple of 4 (device handle: 0x%p)\n", dwOffset, hDev);
        return WD_INVALID_PARAMETER;
    }
    ...
    UINT32 *pu32Data, DWORD EEPROMmsb)
{
    ...
    if (dwOffset % 4)
    {
        ErrLog("PLX_EEPROM_RT_Read32: Error - offset (0x%lX) is not a multiple "
    ...
    UINT32 u32Data, DWORD EEPROMmsb)
{
    ...
    if (dwOffset % 4)
    {
        ErrLog("PLX_EEPROM_RT_Write32: Error - offset (0x%lX) is not a "
            "multiple of 4 (device handle: 0x%p)\n", dwOffset, hDev);
        return WD_INVALID_PARAMETER;
    }
