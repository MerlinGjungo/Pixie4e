    ...
    const WD_PCI_SLOT *pSlot)
{
    ...
    {
        ErrLog("PLX6466_DeviceOpenBySlot: Failed retrieving the device's "
    ...
    pDevCtx = (PPLX6466_DEV_CTX)malloc(sizeof (PLX6466_DEV_CTX));
    if (!pDevCtx)
    {
        ErrLog("PLX6466_DeviceOpenBySlot: Failed allocating memory for PLX "
            "device context\n");
    ...
    *pDeviceHandle = hDev;
    ...
    dwStatus = DeviceInit(hDev);
    if (dwStatus)
        goto Error;

    /* Return handle to the new device */
    TraceLog("PLX6466_DeviceOpenBySlot: Opened a PLX device (handle 0x%p)\n",
        hDev);
    return dwStatus;
    ...
    DWORD dwDeviceID, DWORD nCardNum)
{
    ...
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        ErrLog("PLX6466_DeviceOpen: Failed scanning the PCI bus.\n"
            "Error: 0x%lx - %s\n", dwStatus, Stat2Str(dwStatus));
        return dwStatus;
    }
    ...
    {
        ErrLog("PLX6466_DeviceOpen: No matching PLX device was found for "
            "search criteria (Vendor ID 0x%lX, Device ID 0x%lX, Card Nmuber "
    ...
    return PLX6466_DeviceOpenBySlot(pDeviceHandle, slot);
}
...
static void PLX6466_EventHandler(WD_EVENT *pEvent, PVOID pData)
{
    ...
    TraceLog("PLX6466_EventHandler entered, pData 0x%p, dwAction 0x%lx\n",
    ...
    UINT32 *pu32Data)
{
    ...
    if (dwOffset % 4)
    {
        ErrLog("PLX6466_EEPROM_VPD_Read32: Error - offset (0x%lX) is not a "
    ...
    UINT32 u32Data)
{
    ...
    if (dwOffset % 4)
    {
        ErrLog("PLX6466_EEPROM_VPD_Write32: Error - offset (0x%lX) is not a "
            "multiple of 4 (device handle: 0x%p)\n", dwOffset, hDev);
        return WD_INVALID_PARAMETER;
    }
