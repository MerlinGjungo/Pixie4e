...
char *SectionToString(DWORD dwSection)
{
    ...
    if (dwSection & S_ISAPNP)
    {
        strcat (sSection, "ISAPNP ");
        dwSection &= ~S_ISAPNP;
    }
...
{
    fprintf(file_h, "%s v%ld.%02ld installed (%s)\n", sDriverName,
...
void Print_status(HANDLE hWD)
{
    ...
    fprintf(file_h, "Debug level (%ld) %s, Debug sections (0x%08x) %s, Buffer "
...
int main(int argc, char *argv[])
{
    ...
    if (dump_to_file || !stricmp(argv[1], "dump"))
    {
        ...
        fprintf(file_h, "OS: Windows %s %ld.%ld Build %ld.%ld.%ld %s\n",
            sOSName,
...
#elif defined(LINUX)
        {
            ...
            {
#if defined(LINUX)
                fprintf(file_h, "OS: Linux\n");
#endif
            }
            else
            {
                fprintf(file_h, "OS: %s %s %s %s\n", ver.sysname, ver.release,
                    ver.version, ver.machine);
            }
        }
#else
        fprintf(file_h, "OS: Unknown\n");
#endif
#if defined(UNIX)
    ...
    else if (stricmp(argv[1], "on") == 0 || stricmp(argv[1], "dbg_on") == 0)
    {
        ...
        if (stricmp(argv[1], "dbg_on") == 0)
        {
            ...
            /* Check if running on Windows Vista or higher */
            if (os_type == OS_WIN_NT_6 || os_type == OS_WIN_NT_10 )
            {
