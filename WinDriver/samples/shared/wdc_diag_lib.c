    ...
    FILE *fp)
{
    ...
    for (i = 0, resources = 0; i < pCard->dwItems; i++)
    {
        ...
        case ITEM_MEMORY:
            resources++;
            fprintf(fp, "    Memory range [BAR %ld]: base 0x%"PRI64"X, "
                "size 0x%"PRI64"X\n", pItem->I.Mem.dwBar,
    ...
    DWORD dwAddrSpace, WDC_ADDR_MODE mode)
{
    ...
    dwStatus = DIAG_InputDWORD(&dwOffset, (WDC_READ == direction) ?
    ...
    if (DIAG_INPUT_SUCCESS != dwStatus)
    {
        WDC_DIAG_ERR("WDC_DIAG_ReadWriteAddr: %s getting the offset\n",
            dwStatus == DIAG_INPUT_CANCEL ? "Canceled" : "Failed");
        return;
    }
    ...
    DWORD dwAddrSpace)
{
    ...
    dwStatus = DIAG_InputDWORD(&dwOffset, "offset", TRUE, 0, 0);
    if (DIAG_INPUT_SUCCESS != dwStatus)
    {
    ...
    dwStatus = DIAG_InputDWORD(&dwBytes, "bytes", TRUE, 0, 0);
    if (DIAG_INPUT_SUCCESS != dwStatus)
    {
        WDC_DIAG_ERR("WDC_DIAG_ReadWriteBlock: %s getting the number of bytes "
            "to transfer\n", dwStatus == DIAG_INPUT_CANCEL ? "Canceled" :
            "Failed");
        return;
    }
    ...
    else /* Read/write a memory or I/O address */
    {
        ...
        printf("Do you wish to increment the address after each %s block "
            "(0x%lX bytes) (0 - No, Otherwise - Yes)? ", sDir,
            WDC_ADDR_MODE_TO_SIZE(mode));
        fgets(gsInput, sizeof(gsInput), stdin);
    ...
    DWORD dwNumRegs, BOOL fPciCfg, BOOL fExpress)
{
    ...
    for (i = 1, pReg = pRegs; i <= dwNumRegs; i++, pReg++)
    {
        printf("%2ld. %-*.*s ", i, MAX_NAME_DISPLAY, MAX_NAME_DISPLAY,
            pReg->sName);
        ...
        dwStatus = fExpress ?
            ...
            PciConfRegData2Str(hDev, pReg->dwOffset, details, sizeof(details),
                &outBufLen);
        if (!strcmp(details, ""))
        {
            printf("\n");
    ...
    DWORD dwNumRegs, WDC_DIRECTION direction, BOOL fPciCfg)
{
    ...
    case WDC_SIZE_8:
        ...
        if (WD_STATUS_SUCCESS == dwStatus)
        {
            printf("%s 0x%X %s register %s at offset [0x%lx]\n",
                (WDC_READ == direction) ? "Read" : "Wrote", (UINT32)bData,
    ...
    case WDC_SIZE_16:
        ...
        if (WD_STATUS_SUCCESS == dwStatus)
        {
            printf("%s [0x%hX] %s register %s at offset [0x%lx]\n",
                (WDC_READ == direction) ? "Read" : "Wrote", wData,
                (WDC_READ == direction) ? "from" : "to", pReg->sName,
                fExpress ? pReg->dwOffset + pciExpressOffset : pReg->dwOffset);
            dwStatus = fExpress ?
    ...
    case WDC_SIZE_64:
        ...
        if (WD_STATUS_SUCCESS == dwStatus)
        {
            printf("%s [0x%"PRI64"X] %s register %s at offset [0x%lx]\n",
                (WDC_READ == direction) ?  "Read" : "Wrote", u64Data,
                (WDC_READ == direction) ? "from" : "to", pReg->sName,
                fExpress ? pReg->dwOffset + pciExpressOffset : pReg->dwOffset);
            dwStatus = fExpress ?
...
void WDC_DIAG_ScanPCICapabilities(WDC_DEVICE_HANDLE hDev)
{
    ...
    if (option == 2 || option == 4)
    {
        printf("Enter requested %scapability ID (hexadecimal): 0x",
            option == 4 ? "extended " : "");
        fgets(gsInput, sizeof(gsInput), stdin);
        if (sscanf(gsInput, "%lx", &dwCapId) < 1)
        {
            printf("Invalid input\n");
            return;
        }
    ...
    {
        printf("    %ld) %s - ID [0x%lx], offset [0x%lx]\n", i + 1,
            (option == 3 || option == 4) ?
    ...
    BOOL dump_cfg)
{
    ...
    dwExpressGen = WDC_GetPciExpressGenBySlot(pPciSlot);
    if (dwExpressGen)
