...
BOOL PCI_Get_WD_handle(HANDLE *phWD)
{
    ...
    *phWD = WD_Open();
    ...
    if (ver.dwVer < WD_VER)
    {
        printf("Incorrect " WD_PROD_NAME " version. Expected %d.%02d, "
            "got %ld.%02ld\n", WD_VER / 100, WD_VER % 100, ver.dwVer / 100,
            ver.dwVer % 100);
        goto Error;
    }
...
void PCI_EditConfigReg(WD_PCI_SLOT pciSlot)
{
    ...
    do {
        ...
        {
            i = cmd - 1;
            printf("Enter value (hex format) to write to %s register "
                "(or 'X' to cancel): ", fields[i].name);
            fgets(line, sizeof(line), stdin);
            if (toupper(line[0]) != 'X')
            {
                UINT32 dwVal;
