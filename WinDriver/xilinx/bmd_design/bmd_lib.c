...
WDC_DEVICE_HANDLE BMD_DeviceOpen(const WD_PCI_CARD_INFO *pDeviceInfo,
    char *kp_name)
{
    DWORD dwStatus;
    PBMD_DEV_CTX pDevCtx = NULL;
    WDC_DEVICE_HANDLE hDev = NULL;
    BMD_DEV_ADDR_DESC devAddrDesc;
    PWDC_DEVICE pDev;

    /* Validate arguments */
    if (!pDeviceInfo)
    {
        ErrLog("BMD_DeviceOpen: Error - NULL device information struct "
            "pointer\n");
        return NULL;
    }

    /* Allocate memory for the BMD device context */
    pDevCtx = (PBMD_DEV_CTX)calloc(1, sizeof(BMD_DEV_CTX));
    if (!pDevCtx)
    {
        ErrLog("Failed allocating memory for BMD device context\n");
        return NULL;
    }
    ...
    dwStatus = WDC_KernelPlugInOpen(hDev,
        kp_name && *kp_name ? kp_name : KP_BMD_DRIVER_NAME, &devAddrDesc);
    if (WD_STATUS_SUCCESS != dwStatus)
        TraceLog("BMD_DeviceOpen: Failed opening KernelPlugin\n");

    /* Validate device information */
    if (!DeviceValidate((PWDC_DEVICE)hDev))
        goto Error;

    TraceLog("BMD_DeviceOpen: Opened a BMD device (handle 0x%p)\n"
        "Device is %s using a Kernel PlugIn driver (%s)\n", hDev,
        (WDC_IS_KP(hDev))? "" : "not" , KP_BMD_DRIVER_NAME);
...
BOOL BMD_DeviceClose(WDC_DEVICE_HANDLE hDev)
{
    ...
    TraceLog("BMD_DeviceClose: Entered. Device handle [0x%p]\n", hDev);

    /* Validate the WDC device handle */
    if (!hDev)
    {
        ErrLog("BMD_DeviceClose: Error - NULL device handle\n");
        return FALSE;
    }
    ...
    /* Free BMD device context memory */
    if (pDevCtx)
        free(pDevCtx);
...
WORD BMD_DmaGetMaxPacketSize(WDC_DEVICE_HANDLE hDev, BOOL fIsRead)
{
    ...
    /* Validate the WDC device handle */
    if (!IsValidDevice(hDev, "BMD_DmaGetMaxPacketSize"))
        return 0;
...
BOOL BMD_DmaIsDone(WDC_DEVICE_HANDLE hDev, BOOL fIsRead)
{
    ...
    /* Validate the WDC device handle */
    if (!IsValidDevice(hDev, "BMD_DmaIsDone"))
        return FALSE;
...
BOOL BMD_DmaIntEnable(WDC_DEVICE_HANDLE hDev, BOOL fIsRead)
{
    ...
    /* Validate the WDC device handle */
    if (!IsValidDevice(hDev, "BMD_DmaIntEnable"))
        return FALSE;
...
BOOL BMD_DmaIntDisable(WDC_DEVICE_HANDLE hDev, BOOL fIsRead)
{
    ...
    /* Validate the WDC device handle */
    if (!IsValidDevice(hDev, "BMD_DmaIntDisable"))
        return FALSE;
...
BOOL BMD_DmaIsReadSucceed(WDC_DEVICE_HANDLE hDev)
{
    ...
    /* Validate the WDC device handle */
    if (!IsValidDevice(hDev, "BMD_DmaIsReadSucceed"))
        return FALSE;
...
static void BMD_EventHandler(WD_EVENT *pEvent, PVOID pData)
{
    ...
    TraceLog("BMD_EventHandler: Entered. pData [0x%p], dwAction [0x%lx]\n",
...
DWORD BMD_GetNumAddrSpaces(WDC_DEVICE_HANDLE hDev)
{
    ...
    /* Validate the WDC device handle */
    if (!IsValidDevice(pDev, "BMD_GetNumAddrSpaces"))
        return 0;
    ...
    BMD_ADDR_SPACE_INFO *pAddrSpaceInfo)
{
    ...
    /* Validate the WDC device handle */
    if (!IsValidDevice(pDev, "BMD_GetAddrSpaceInfo"))
        return FALSE;
    ...
    {
        ErrLog("BMD_GetAddrSpaceInfo: Error - Address space %ld "
