...
int main(void)
{
    ...
    hDev = DeviceFindAndOpen(XDMA_DEFAULT_VENDOR_ID, XDMA_DEFAULT_DEVICE_ID);

    /* Get the configuration BAR number */
    dwConfigBarNum = XDMA_ConfigBarNumGet(hDev);
...
static void MenuMain(WDC_DEVICE_HANDLE *phDev)
{
    ...
    {
        printf("\n");
        printf("XDMA main menu\n");
        printf("--------------\n");
        printf("%d. Scan PCI bus\n", MENU_MAIN_SCAN_PCI_BUS);
        printf("%d. Find and open a XDMA device\n", MENU_MAIN_FIND_AND_OPEN);
        if (*phDev)
        {
        ...
        switch (option)
        {
            ...
            case MENU_MAIN_SCAN_PCI_BUS: /* Scan PCI bus */
                ...
                break;
            case MENU_MAIN_FIND_AND_OPEN: /* Find and open a XDMA device */
                if (*phDev)
                    DeviceClose(*phDev);
...
static BOOL DeviceFind(DWORD dwVendorId, DWORD dwDeviceId, WD_PCI_SLOT *pSlot)
{
    ...
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        XDMA_ERR("DeviceFind: Failed scanning the PCI bus.\n"
    ...
    if (!dwNumDevices)
    {
        XDMA_ERR("No matching device was found for search criteria "
...
/* Open a handle to a XDMA device */
static WDC_DEVICE_HANDLE DeviceOpen(const WD_PCI_SLOT *pSlot)
{
    ...
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        XDMA_ERR("DeviceOpen: Failed retrieving the device's resources "
            "information.\nError 0x%lx - %s\n", dwStatus, Stat2Str(dwStatus));
        return NULL;
    }
    ...
    hDev = XDMA_DeviceOpen(&deviceInfo, NULL);
    if (!hDev)
    {
        XDMA_ERR("DeviceOpen: Failed opening a handle to the device: %s\n",
            XDMA_GetLastErr());
        return NULL;
    }
...
/* Close a handle to a XDMA device */
static void DeviceClose(WDC_DEVICE_HANDLE hDev)
{
    ...
    if (!XDMA_DeviceClose(hDev))
    {
        XDMA_ERR("DeviceClose: Failed closing XDMA device: %s",
            XDMA_GetLastErr());
    }
}
...
static void MenuReadWriteAddr(WDC_DEVICE_HANDLE hDev)
{
    ...
    {
        DWORD dwNumAddrSpaces = XDMA_GetNumAddrSpaces(hDev);
        ...
        {
            XDMA_ERR("MenuReadWriteAddr: Error - no active address spaces "
                "found\n");
...
static void SetAddrSpace(WDC_DEVICE_HANDLE hDev, PDWORD pdwAddrSpace)
{
    ...
    for (dwAddrSpace = 0; dwAddrSpace < dwNumAddrSpaces; dwAddrSpace++)
    {
        XDMA_ADDR_SPACE_INFO addrSpaceInfo;

        BZERO(addrSpaceInfo);
        addrSpaceInfo.dwAddrSpace = dwAddrSpace;
        if (!XDMA_GetAddrSpaceInfo(hDev, &addrSpaceInfo))
        {
            XDMA_ERR("SetAddrSpace: Error - Failed to get address space "
                "information: %s", XDMA_GetLastErr());
            return;
        }
...
static void MenuReadWriteCfgSpace(WDC_DEVICE_HANDLE hDev)
{
    ...
    do {
        ...
        printf("------------------------\n");
        WDC_DIAG_RegsInfoPrint(hDev, gXDMA_CfgRegs, XDMA_CFG_REGS_NUM,
        ...
        if (DIAG_INPUT_FAIL == DIAG_GetMenuOption(&option,
            XDMA_CFG_REGS_NUM ? MENU_RW_CFG_SPACE_WRITE_REG :
            MENU_RW_CFG_SPACE_WRITE_OFFSET))
        {
            continue;
        }
        ...
        switch (option)
        {
            ...
            case MENU_RW_CFG_SPACE_READ_ALL_REGS:
                WDC_DIAG_ReadRegsAll(hDev, gXDMA_CfgRegs, XDMA_CFG_REGS_NUM,
                    TRUE, FALSE);
                break;
            case MENU_RW_CFG_SPACE_READ_REG:  /* Read from a configuration
                                                 register */
                WDC_DIAG_ReadWriteReg(hDev, gXDMA_CfgRegs, XDMA_CFG_REGS_NUM,
                    WDC_READ, TRUE);
                break;
...
static void MenuReadWriteConfigRegs(WDC_DEVICE_HANDLE hDev)
{
    ...
    do {
        /* Display predefined registers information */
        printf("\n");
        printf("XDMA config block registers:\n");
        printf("--------------------------\n");
        WDC_DIAG_RegsInfoPrint(hDev, gXDMA_ConfigRegs, XDMA_CONFIG_REGS_NUM,
            WDC_DIAG_REG_PRINT_ALL, FALSE);

        printf("\n");
        printf("Read/write the XDMA config block registers\n");
        printf("-----------------------------------------\n");
        ...
        switch (option)
        {
            ...
            case MENU_RW_REGS_READ_ALL:
                WDC_DIAG_ReadRegsAll(hDev, gXDMA_ConfigRegs,
                    XDMA_CONFIG_REGS_NUM, FALSE, FALSE);
                break;
            case MENU_RW_REGS_READ_REG:  /* Read from a register */
                WDC_DIAG_ReadWriteReg(hDev, gXDMA_ConfigRegs,
                    XDMA_CONFIG_REGS_NUM, WDC_READ, FALSE);
                break;
            case MENU_RW_REGS_WRITE_REG: /* Write to a register */
                WDC_DIAG_ReadWriteReg(hDev, gXDMA_ConfigRegs,
                    XDMA_CONFIG_REGS_NUM, WDC_WRITE, FALSE);
                break;
        }
        ...
        switch (option)
        {
            ...
            case MENU_DMA_SINGLE_OPEN_CLOSE: /* Open/close DMA menu option */
                if (!fIsDmaOpen)
                {
                    ...
                    hDma = XDMA_DIAG_DmaOpen(hDev, fPolling, dwChannel,
...
static void MenuDmaPerformance(WDC_DEVICE_HANDLE hDev)
{
    ...
    do {
        ...
        XDMA_DIAG_DmaPerformance(hDev, option, dwBytes, fPolling, dwSeconds);
...
static void MenuEvents(WDC_DEVICE_HANDLE hDev)
{
    ...
    do
    {
        fRegister = !XDMA_EventIsRegistered(hDev);
            ...
            case MENU_EVENTS_REGISTER_UNREGISTER: /* Register/unregister events
                ...
                if (fRegister)
                {
                    if (WD_STATUS_SUCCESS ==
                        XDMA_EventRegister(hDev, DiagEventHandler))
                    {
                        printf("Events registered\n");
                    }
                    else
                    {
                        XDMA_ERR("Failed to register events. Last error:\n%s",
                            XDMA_GetLastErr());
                    }
                }
                else
                {
                    if (WD_STATUS_SUCCESS == XDMA_EventUnregister(hDev))
                        printf("Events unregistered\n");
                    else
                    {
                        XDMA_ERR("Failed to unregister events. Last Error:\n%s",
                            XDMA_GetLastErr());
                    }
                }
                break;
