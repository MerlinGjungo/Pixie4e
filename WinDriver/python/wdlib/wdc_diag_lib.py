...
class WDC_REG(Structure): _fields_ =\
    [("dwAddrSpace", DWORD), # Number of address space in which the register
                             # resides
                             # For PCI configuration registers, use
                             # WDC_AD_CFG_SPACE
    ("dwOffset", DWORD),       # Offset of the register in the dwAddrSpace
                             # address space
    ("dwSize", DWORD),            # Register's size (in bytes)
    ("direction", WDC_DIRECTION), # Read/write access mode - see WDC_DIRECTION
                                  # options
    ("sName", CHAR * MAX_NAME),   # Register's name
    ("sDesc", CHAR * MAX_DESC)]   # Register's description
...
def WDC_DIAG_ReadWriteAddr(hDev, direction, dwAddrSpace, mode):
    ...
    if WD_STATUS_SUCCESS != dwStatus:
        print("Failed to %s offset 0x%lX in BAR %ld. Error 0x%lx - %s\n" %
            ...
            dwAddrSpace, dwStatus, wdapi.Stat2Str(dwStatus)))
...
def WDC_DIAG_ReadWriteBlock(hDev, direction, dwAddrSpace):
    ...
    if WD_STATUS_SUCCESS == dwStatus:
        ...
        print("Failed to %s 0x%lX bytes %s offset 0x%lX. Error 0x%lx - %s\n" %
            ...
            dwOffset.value, dwStatus, wdapi.Stat2Str(dwStatus)))
...
def WDC_DIAG_ReadRegsAll(hDev, pRegs, dwNumRegs, fPciCfg, fExpress):
    ...
    for i, pReg in enumerate(pRegs, 1):
        ...
        if outBufLen.value > 0:
            WDC_DIAG_PrintIndent(details.value.strip() + "\n",
...
def WDC_DIAG_ReadWriteReg(hDev, pRegs, dwNumRegs, direction, fPciCfg):
    ...
    if WDC_READ == direction:
        dwStatus = \
            ...
            wdapi.PciConfRegData2Str(hDev, pReg.dwOffset, details,
                sizeof(details), byref(outBufLen))
        print("Decoded register data:\n%s" % (details.value if len(details)
            else ""))
...
def WDC_DIAG_PciDeviceInfoPrintFile(pPciSlot, fp, dump_cfg):
    ...
    dwExpressGen = wdapi.WDC_GetPciExpressGenBySlot(byref(pPciSlot))
    if (dwExpressGen != 0):
