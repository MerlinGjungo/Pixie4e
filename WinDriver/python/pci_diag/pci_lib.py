...
def TraceLog(s):
    wdapi_va.WDC_Trace("PCI lib TRACE: %s\n" % s )
...
MAX_TYPE = 8
class PCI_ADDR_SPACE_INFO(Structure): _fields_ = \
    [("dwAddrSpace", DWORD),
    ("sType", CHAR * MAX_TYPE),
    ("sName", CHAR * MAX_NAME),
    ("sDesc", CHAR * MAX_DESC)]
...
#        byref(gpDevCtx))
    ...
    wdapi.WDC_KernelPlugInOpen(hDev, KP_PCI_DRIVER_NAME, byref(devAddrDesc))
...
def PCI_IntHandler(pData):
    ...
    pDevCtx = cast(wdapi.WDC_GetDevContext(pData), PPCI_DEV_CTX)
...
def PCI_IntEnable(hDev, funcIntHandler):
    ...
    dwStatus = wdapi.WDC_IntEnable(hDev, gpTrans, dwNumTransCmds, dwOptions,
        DLLCALLCONV(None, PVOID)(PCI_IntHandler), hDev, WDC_IS_KP(pDev))
    if WD_STATUS_SUCCESS != dwStatus:
...
def PCI_GetNumAddrSpaces(hDev):
    pDev = cast(hDev, PWDC_DEVICE)
    # Validate the device handle
    if not IsValidDevice(hDev, "PCI_GetNumAddrSpaces"):
        return 0
...
def PCI_GetAddrSpaceInfo(hDev, addrSpaceInfo):
    pDev = cast(hDev, PWDC_DEVICE)
