...
WDC_DEVICE_HANDLE QSYS_DeviceOpen(WD_PCI_CARD_INFO *pDeviceInfo,
        char* kp_name)
{
    DWORD dwStatus;
    PQSYS_DEV_CTX pDevCtx = NULL;
    WDC_DEVICE_HANDLE hDev = NULL;
    QSYS_DEV_ADDR_DESC devAddrDesc;
    PWDC_DEVICE pDev;

    /* Validate arguments */
    if (!pDeviceInfo)
    {
        ErrLog("QSYS_DeviceOpen: Error - NULL device information struct "
            "pointer\n");
        return NULL;
    }

    /* Allocate memory for the Qsys device context */
    pDevCtx = (PQSYS_DEV_CTX)malloc(sizeof(QSYS_DEV_CTX));
    if (!pDevCtx)
    {
        ErrLog("Failed allocating memory for Qsys device context\n");
        return NULL;
    }
    ...
    dwStatus = WDC_KernelPlugInOpen(hDev,
        kp_name && *kp_name ? kp_name : KP_QSYS_DRIVER_NAME, &devAddrDesc);
    if (WD_STATUS_SUCCESS != dwStatus)
        TraceLog("QSYS_DeviceOpen: Failed opening KernelPlugin\n");

    /* Validate device information */
    if (!DeviceValidate((PWDC_DEVICE)hDev))
        goto Error;

    TraceLog("QSYS_DeviceOpen: Opened a Qsys device (handle 0x%p)\n"
        "Device is %s using a Kernel PlugIn driver (%s)\n", hDev,
        (WDC_IS_KP(hDev))? "" : "not" , KP_QSYS_DRIVER_NAME);
...
BOOL QSYS_DeviceClose(WDC_DEVICE_HANDLE hDev)
{
    ...
    /* Free Qsys device context memory */
    if (pDevCtx)
        free(pDevCtx);
...
BOOL QSYS_DmaIsDone(QSYS_PDMA_STRUCT pDma, UINT32 u32ExpectedData)
{
    ...
    /* Validate the WDC device handle */
    if (!IsValidDevice(pDma->hDev, "QSYS_DmaIsDone"))
        return FALSE;
...
BOOL QSYS_DmaTransferVerify(WDC_DEVICE_HANDLE hDev)
{
    ...
    /* Validate the WDC device handle */
    if (!IsValidDevice(hDev, "QSYS_DmaTransferVerify"))
        return FALSE;
...
static void QSYS_EventHandler(WD_EVENT *pEvent, PVOID pData)
{
    ...
    TraceLog("QSYS_EventHandler entered, pData: 0x%p, dwAction 0x%lx\n",
...
DWORD QSYS_GetNumAddrSpaces(WDC_DEVICE_HANDLE hDev)
{
    ...
    /* Validate the WDC device handle */
    if (!IsValidDevice(pDev, "QSYS_GetNumAddrSpaces"))
        return 0;
    ...
    QSYS_ADDR_SPACE_INFO *pAddrSpaceInfo)
{
    ...
    /* Validate the WDC device handle */
    if (!IsValidDevice(pDev, "QSYS_GetAddrSpaceInfo"))
        return FALSE;

#if defined(DEBUG)
    if (!pAddrSpaceInfo)
    {
        ErrLog("QSYS_GetAddrSpaceInfo: Error - NULL address space information"
            " pointer\n");
        return FALSE;
    }
#endif
    ...
    {
        ErrLog("QSYS_GetAddrSpaceInfo: Error - Address space %ld is out of "
