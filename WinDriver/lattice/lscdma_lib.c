...
/* -----------------------------------------------
    Device open/close
   ----------------------------------------------- */
WDC_DEVICE_HANDLE LSCDMA_DeviceOpen(const WD_PCI_CARD_INFO *pDeviceInfo)
{
    DWORD dwStatus;
    PLSCDMA_DEV_CTX pDevCtx = NULL;
    WDC_DEVICE_HANDLE hDev = NULL;
    LSCDMA_DEV_ADDR_DESC devAddrDesc;
    PWDC_DEVICE pDev;

    /* Validate arguments */
    if (!pDeviceInfo)
    {
        ErrLog("LSCDMA_DeviceOpen: Error - NULL device information struct pointer\n");
        return NULL;
    }

    /* Allocate memory for the LSCDMA device context */
    pDevCtx = (PLSCDMA_DEV_CTX)malloc(sizeof(LSCDMA_DEV_CTX));
    if (!pDevCtx)
    {
        ErrLog("Failed allocating memory for LSCDMA device context\n");
        return NULL;
    }
    ...
    WDC_KernelPlugInOpen(hDev, KP_LSCDMA_DRIVER_NAME, &devAddrDesc);

    /* Init SGDMA device */
    if (!DeviceInit((PWDC_DEVICE)hDev))
        goto Error;

    /* Return handle to the new device */
    TraceLog("LSCDMA_DeviceOpen: Opened a LSCDMA device (handle 0x%p)\n"
        "Device is %susing a Kernel PlugIn driver %s\n", hDev,
        WDC_IS_KP(hDev) ? "" : "not " ,
        WDC_IS_KP(hDev) ? KP_LSCDMA_DRIVER_NAME : "");
    return hDev;
...
BOOL LSCDMA_DeviceClose(WDC_DEVICE_HANDLE hDev)
{
    ...
    /* Free LSCDMA device context memory */
    if (pDevCtx)
        free(pDevCtx);
    ...
    BOOL fToDevice)
{
    ...
    {
        TraceLog("DMA handle already open for channel %ld\n", dwChannel);
...
static void LSCDMA_EventHandler(WD_EVENT *pEvent, PVOID pData)
{
    ...
    TraceLog("LSCDMA_EventHandler entered, pData: 0x%p, dwAction 0x%lx\n",
...
DWORD LSCDMA_GetNumAddrSpaces(WDC_DEVICE_HANDLE hDev)
{
    ...
    if (!IsValidDevice(pDev, "LSCDMA_GetNumAddrSpaces"))
        return 0;
...
BOOL LSCDMA_GetAddrSpaceInfo(WDC_DEVICE_HANDLE hDev, LSCDMA_ADDR_SPACE_INFO *pAddrSpaceInfo)
{
    ...
    if (!IsValidDevice(pDev, "LSCDMA_GetAddrSpaceInfo"))
        return FALSE;

#if defined(DEBUG)
    if (!pAddrSpaceInfo)
    {
        ErrLog("LSCDMA_GetAddrSpaceInfo: Error - NULL address space information pointer\n");
        return FALSE;
    }
#endif
    ...
    {
        ErrLog("LSCDMA_GetAddrSpaceInfo: Error - Address space %ld is out of range (0 - %ld)\n",
