...
#include "linux_common.h"
    ...
    /* If CONFIG_AMD_MEM_ENCRYPT is defined it causes a GPL compilation error
     * when trying to compile WinDriver */
...
REGISTER_FUNC_MOD kp_register_mod_func_t %DRIVER_NAME%_register_kp_module_func;
#else
REGISTER_FUNC_MOD kp_register_mod_func_t windrvr1281_register_kp_module_func;
#endif
...
#endif
{
    ...
    unsigned long va = (unsigned long)private_data->kernel_addr + off;
    struct page *page;
...
static inline int uncached_access(struct file *file, unsigned long addr)
{
    ...
    static int wd_mmap(struct file *file, struct vm_area_struct *vma)
    {
        ...
        else
        {
            ...
            if (REMAP_PAGE_RANGE(REMAP_ARG(vma) vma->vm_start, REMAP_OFFSET(vma),
...
const char *LINUX_get_driver_name(void)
{
    ...
    #else
        return windrvr1281_register_kp_module_func;
    #endif
    }
    ...
    #else
        unix_get_register_kp_module(&(windrvr1281_register_kp_module_func));
    #endif
    }
#endif
...
#endif
{
    return wd_intr_handler_linux(ctx);
}
...
#else
EXPORT_SYMBOL(windrvr1281_register_kp_module_func);
#endif
...
#else
    #define WD_PCI_DRIVER_NAME "windrvr1281_pci"
#endif
...
DWORD pci_get_supported_interrupt_types(DWORD bus, DWORD slot, DWORD func);
void LINUX_pci_get_pnp_data(void *dev_h, LINUX_pnp_data *p)
{
    struct pci_dev *dev = (struct pci_dev *)dev_h;

    if ((pci_get_supported_interrupt_types(dev->bus->number,
        PCI_SLOT(dev->devfn), PCI_FUNC(dev->devfn)) != 0))
    {
        p->irq = dev->irq;
    }
    else
...
long LINUX_get_time_in_millisec()
{
    ...
    millisec = tv.tv_usec / 1000; /* Converting microseconds to milliseconds */
...
{
    pci_dma_sync_single_for_cpu(dev, (dma_addr_t)dma_addr, size,
        (int)dma_direction);
}
    ...
    unsigned int dma_direction)
{
    ...
    for (i = 0; i < nelems; i++)
    {
        pci_dma_sync_single_for_cpu(dev, sg_dma_address(&sgl[i]),
            sg_dma_len(&sgl[i]), (int)dma_direction);
    }
#endif
}
    ...
    unsigned int dma_direction)
{
    ...
    for (i = 0; i < nelems; i++)
    {
        pci_dma_sync_single_for_device(dev, sg_dma_address(&sgl[i]),
            sg_dma_len(&sgl[i]), (int)dma_direction);
    }
#endif
}
...
static int build_dma_list_26(LINUX_dma_page *page_list, void *buf,
    unsigned long size, unsigned int *dma_sglen, unsigned int dma_direction,
    void *dev_handle, void **dma_handle)
{
    int rc, i, offset;
    struct page **pages = NULL;
    ...
    *dma_sglen = page_count;
    mask = dev_handle ?
        ((struct pci_dev *)dev_handle)->dma_mask : (dma_addr_t) -1;
#else
    *dma_sglen = pci_map_sg((struct pci_dev *)dev_handle, sgl, page_count,
    ...
    for (i = 0; i < *dma_sglen; i++)
    {
        ...
        dma_addr = (dma_addr_t)LINUX_dma_map_single(dev_handle, va,
    ...
    void **dma_handle)
{
    ...
    rc = build_dma_list_26(page_list, buf, size, &page_count,
        (int)dma_direction, dev_handle, dma_handle);
    if (!rc)
        *dma_sglen = page_count;
    ...
    unsigned int dma_direction, void *dev_handle)
{
    ...
    {
#if defined(_CONFIG_SWIOTLB)
        pci_unmap_single(dev_handle, sg_dma_address(&sgl[i]),
            sg_dma_len(&sgl[i]), (int)dma_direction);
#endif
...
int OS_pci_sriov_vf_get_owner(void *pdev, unsigned long *dwBus,
    unsigned long *dwSlot, unsigned long *dwFunc)
{
    printk("%s: Not supported on this platform\n", __FUNCTION__);
    return WD_NOT_IMPLEMENTED;
}
...
int OS_pci_sriov_vf_get_owner(void *pdev, unsigned long *dwBus,
    unsigned long *dwSlot, unsigned long *dwFunc)
{
    return WD_SRIOV_FUNC_NAME(OS_pci_sriov_vf_get_owner)(pdev, dwBus, dwSlot,
        dwFunc);
}
