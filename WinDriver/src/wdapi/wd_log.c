...
static void log_WD_PCI_CAP(WD_PCI_CAP *p, int ident)
{
    ...
    LOG(ident + 1, "dwCapId [0x%lx], dwCapOffset [0x%lx]\n", p->dwCapId,
        p->dwCapOffset);
}
...
{
    LOG(ident, "WD_ITEMS:\n");
    LOG(ident + 1, "item=%lx, fNotSharable=%lx\n",
    ...
    case ITEM_MEMORY:
        ...
        LOG(ident + 2, "pPhysicalAddr=%"PRI64"x, dwBytes=%"PRI64"x, "
            "dwTransAddr=%x:%x, dwUserDirectAddr=%lx, dwBar=%lx, dwOptions=%lx"
...
#else
        LOG(ident + 2, "pPhysicalAddr=%"PRI64"x, dwBytes=%"PRI64"x, "
            "dwTransAddr=%lx, dwUserDirectAddr=%lx, dwBar=%lx, dwOptions=%lx\n",
...
static void log_WD_IPC_PROCESS(WD_IPC_PROCESS *p, int ident)
{
    ...
    LOG(ident + 1, "dwSubGroupID=0x%lx\n", p->dwSubGroupID);
    LOG(ident + 1, "dwGroupID=0x%lx\n", p->dwGroupID);
    LOG(ident + 1, "hIpc=%ld\n", p->hIpc);
}
...
#if defined(KERNEL_64BIT)
    /* TODO: use KPRI syntax to print KPTR */
    LOG(ident + 1, "pPort=%x:%x, cmdTrans=%lx, dwBytes=%lx, fAutoinc=%lx, "
        "dwOptions=%lx\n",
        (UINT32)(p->pPort >> 32), (UINT32)(p->pPort), p->cmdTrans, p->dwBytes,
        p->fAutoinc, p->dwOptions);
#else
    LOG(ident + 1, "pPort=%lx, cmdTrans=%lx, dwBytes=%lx, fAutoinc=%lx, "
        "dwOptions=%lx\n",
    ...
    case IOCTL_WD_IPC_SEND:
        {
            ...
            LOG(ident + 1, "hIpc=%ld, dwOptions=0x%lx, dwRecipientID=0x%lx, "
                "dwMsgID=0x%lx, qwMsgData=0x%llx\n", p->hIpc, p->dwOptions,
...
#endif
            LOG(ident + 1, "dwBytes=%lx, dwOptions=%lx, dwPages=%lx, "
    ...
    case IOCTL_WD_EVENT_SEND:
        {
            ...
            LOG(ident + 1, "hEvent=%lx, dwEventType=%lx, dwAction=%lx, "
                "dwEventId=%lx\n", p->hEvent, p->dwEventType, p->dwAction,
                p->dwEventId);
            LOG(ident + 1, "hKernelPlugIn=%lx, dwOptions=%lx\n",
            ...
            case WD_EVENT_TYPE_IPC:
                ...
                LOG(ident + 2, "hIpc=%lx, dwSubGroupID=%lx, dwGroupID=%lx, "
                    "dwSenderUID=%lx, dwMsgID=%lx, qwMsgData=%llx\n",
            ...
            LOG(ident + 1, "dwNumMatchTables=%lx\n", p->dwNumMatchTables);
    ...
    case IOCTL_WD_INT_WAIT:
        {
            ...
            LOG(ident + 1, "fEnableOk=%lx, dwCounter=%lx, dwLost=%lx, "
    ...
    case IOCTL_WD_KERNEL_PLUGIN_OPEN:
        {
            ...
            LOG(ident + 1, "hKernelPlugIn=%lx, cDriverName=%s, "
    ...
    case IOCTL_WD_PCI_CONFIG_DUMP:
        {
            ...
            LOG(ident + 1, "pBuffer=%p, dwOffset=%lx, dwBytes=%lx, "
                "fIsRead=%lx, dwResult=%lx\n", p->pBuffer, p->dwOffset,
    ...
    case IOCTL_WD_PCI_SCAN_CAPS:
        {
            ...
            log_WD_PCI_SLOT(&p->pciSlot, ident + 1);
            LOG(ident + 1, "dwCapID [0x%lx], dwOptions [0x%lx]\n", p->dwCapId,
                p->dwOptions);
            LOG(ident + 1, "dwNumCaps [0x%lx]\n", p->dwNumCaps);
    ...
    case IOCTL_WDU_GET_DEVICE_DATA:
        {
            ...
            LOG(ident + 1, "dwUniqueID=%lx, pBuf=%p, dwBytes=%lx, "
    ...
    case IOCTL_WDU_TRANSFER:
        {
            ...
            LOG(ident + 1, "dwUniqueID=%lx, dwPipeNum=%lx, fRead=%lx, "
                ...
                p->dwOptions);
            LOG(ident + 1, "pBuffer=%p, dwBufferSize=%lx, "
                "dwBytesTransferred=%lx, " "dwTimeout=%lx\n", p->pBuffer,
