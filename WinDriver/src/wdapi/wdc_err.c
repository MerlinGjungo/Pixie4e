...
/* [Initialized to enable error prints in case of debug options initialization
 * failure] */
...
DWORD DLLCALLCONV WDC_SetDebugOptions(WDC_DBG_OPTIONS dbgOptions,
    const CHAR *sDbgFile)
{
    /* Perform any necessary cleanup for previous debug options and set the
    ...
    if (!(dbgOptions & WDC_DBG_LEVEL_ERR) &&
        !(dbgOptions & WDC_DBG_LEVEL_TRACE))
    {
    ...
    if (dbgOptions & WDC_DBG_OUT_FILE)
    {
    #if defined(__KERNEL__)
        sDbgFile = sDbgFile;
        WDC_Err("WDC_SetDebugOptions: Error - WDC_DBG_OUT_FILE (0x%lx) debug "
            "option is only allowed in user-mode.\n");
        return WD_INVALID_PARAMETER;
    #else
        if (sDbgFile && strcmp(sDbgFile, ""))
        {
            ...
            if (!gfpDbgFile)
            {
                WDC_Err("WDC_SetDebugOptions: Debug init: Failed opening debug "
                    "file (%s) for writing.\n"
        ...
        else /* If no file was selected, default debug output file is used */
        {
            gfpDbgFile = DEFAULT_DBG_OUT_FILE;

            if (!gfpDbgFile)
            {
                WDC_Trace("WDC_SetDebugOptions: Debug init: Cannot access "
    ...
    {
        WDC_Err("WDC_SetDebugOptions: Error - You must set either the "
            "WDC_DBG_OUT_DBM or WDC_DBG_OUT_FILE debug flag.\n");
        return WD_INVALID_PARAMETER;
    }
