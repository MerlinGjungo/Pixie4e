...
/* WD_TRANSFER data types defintions (based on WDC address access mode
 * defintions) */
...
static inline DWORD get_trans_cmd(WDC_ADDR_MODE mode, WDC_DIRECTION direction,
    BOOL fBlock, BOOL fMem)
{
    switch (mode)
    {
    case WDC_MODE_8:
    ...
    DWORD dwAddrSpace)
{
    ...
    {
        WDC_Err("WDC_AddrSpaceIsActive: Invalid device handle");
        return FALSE;
    }

    pAddrDesc = WDC_GET_ADDR_DESC(hDev, dwAddrSpace);
    return (BOOL)pAddrDesc->qwBytes;
}
...
{ \
    ...
    \
    BZERO(trans); \
    trans.pPort = (pAddrDesc)->pAddr + dwOffset; \
    trans.dwBytes = bytes; \
...
DWORD DLLCALLCONV WDC_ReadAddr##bits(WDC_DEVICE_HANDLE hDev, \
    DWORD dwAddrSpace, KPTR dwOffset, U##bits *val) \
{ \
    ...
    if (!WdcIsValidDevHandle(hDev)) \
    { \
        WDC_Err("WDC_ReadAddr"#bits": Invalid device handle"); \
...
DWORD DLLCALLCONV WDC_WriteAddr##bits(WDC_DEVICE_HANDLE hDev, \
    DWORD dwAddrSpace, KPTR dwOffset, U##bits val) \
{ \
    DWORD dwStatus = WD_STATUS_SUCCESS; \
    WDC_ADDR_DESC *pAddrDesc; \
    ...
    if (!WdcIsValidDevHandle(hDev)) \
    { \
        WDC_Err("WDC_WriteAddr"#bits": Invalid device handle"); \
        return WD_INVALID_PARAMETER; \
    } \
    \
    pAddrDesc = WDC_GET_ADDR_DESC(hDev, dwAddrSpace); \
    ...
    { \
        READ_WRITE_ADDR_TRANS(pAddrDesc, dwOffset, (PVOID)&val, \
            WDC_MODE_##bits, WDC_MODE_##bits, WDC_WRITE, WDC_ADDR_RW_DEFAULT, \
            dwStatus); \
    } \
    \
    return dwStatus; \
    ...
    WDC_ADDR_RW_OPTIONS options)
{
    ...
    if (!WdcIsValidDevHandle(hDev))
    {
        WDC_Err("WDC_ReadAddrBlock: Invalid device handle");
        return WD_INVALID_PARAMETER;
    }

    pAddrDesc = WDC_GET_ADDR_DESC(hDev, dwAddrSpace);

    READ_WRITE_ADDR_TRANS(pAddrDesc, dwOffset, pData, dwBytes, mode, WDC_READ,
        options, dwStatus);
    return dwStatus;
}
    ...
    WDC_ADDR_RW_OPTIONS options)
{
    ...
    if (!WdcIsValidDevHandle(hDev))
    {
        WDC_Err("WDC_WriteAddrBlock: Invalid device handle");
        return WD_INVALID_PARAMETER;
    }

    pAddrDesc = WDC_GET_ADDR_DESC(hDev, dwAddrSpace);

    READ_WRITE_ADDR_TRANS(pAddrDesc, dwOffset, pData, dwBytes, mode, WDC_WRITE,
        options, dwStatus);
    return dwStatus;
}
...
DWORD DLLCALLCONV WDC_MultiTransfer(WD_TRANSFER *pTrans, DWORD dwNumTrans)
{
    ...
    if (!pTrans || !dwNumTrans)
    {
        WDC_Err("WDC_MultiTransfer: Error - %s\n",
            !pTrans ? "NULL WD_TRANSFER pointer" : "no transfer commands "
            "(dwNumTrans == 0)");
        return WD_INVALID_PARAMETER;
    }
#endif
