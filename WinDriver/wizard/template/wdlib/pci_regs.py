...
HEADER_TYPE_ALL = HEADER_TYPE_NORMAL | HEADER_TYPE_BRIDGE | HEADER_TYPE_CARDBUS

WDC_PCI_HEADER_TYPE = DWORD
...
def PCI_EXT_CAP_ID(header):
    return header & 0x0000ffff

def PCI_EXT_CAP_VER(header):
    return (header >> 16) & 0xf

def PCI_EXT_CAP_NEXT(header):
...
def GET_CAPABILITY_STR(cap_id):
    return "Null Capability" if cap_id == 0x00 else \
    "Power Management" if cap_id == PCI_CAP_ID_PM else \
    "Accelerated Graphics Port" if cap_id == PCI_CAP_ID_AGP else \
    "Vital Product Data" if cap_id == PCI_CAP_ID_VPD else \
    "Slot Identification" if cap_id == PCI_CAP_ID_SLOTID else \
    "Message Signalled Interrupts (MSI)" if cap_id == PCI_CAP_ID_MSI else \
    "CompactPCI HotSwap" if cap_id == PCI_CAP_ID_CHSWP else \
    "PCI-X" if cap_id == PCI_CAP_ID_PCIX else \
    "HyperTransport" if cap_id == PCI_CAP_ID_HT else \
    "Vendor-Specific" if cap_id == PCI_CAP_ID_VNDR else \
    "Debug port" if cap_id == PCI_CAP_ID_DBG else \
    "CompactPCI Central Resource Control" if cap_id == PCI_CAP_ID_CCRC else \
    "PCI Standard Hot-Plug Controller" if cap_id == PCI_CAP_ID_SHPC else \
    "Bridge subsystem vendor/device ID" if cap_id == PCI_CAP_ID_SSVID else \
    "AGP Target PCI-PCI bridge" if cap_id == PCI_CAP_ID_AGP3 else \
    "Secure Device" if cap_id == PCI_CAP_ID_SECDEV else \
    "PCI Express" if cap_id == PCI_CAP_ID_EXP else \
    "Extended Message Signalled Interrupts (MSI-X)" if cap_id == PCI_CAP_ID_MSIX else \
    "SATA Data/Index Conf." if cap_id == PCI_CAP_ID_SATA else \
    "PCI Advanced Features" if cap_id == PCI_CAP_ID_AF else \
    "Unknown"

def GET_EXTENDED_CAPABILITY_STR(cap_id):
    return "Null Capability" if cap_id == 0x0000 else \
    "Advanced Error Reporting (AER)" if cap_id == PCI_EXT_CAP_ID_ERR else \
    "Virtual Channel (VC)" if cap_id == PCI_EXT_CAP_ID_VC else \
    "Device Serial Number" if cap_id == PCI_EXT_CAP_ID_DSN else \
    "Power Budgeting"if cap_id == PCI_EXT_CAP_ID_PWR else \
    "Root Complex Link Declaration"if cap_id == PCI_EXT_CAP_ID_RCLD else \
    "Root Complex Internal Link Control"if cap_id == PCI_EXT_CAP_ID_RCILC else \
    "Root Complex Event Collector Endpoint Association" if cap_id == PCI_EXT_CAP_ID_RCEC else \
    "Multi-Function Virtual Channel (MFVC)" if cap_id == PCI_EXT_CAP_ID_MFVC else \
    "Virtual Channel (VC)" if cap_id == PCI_EXT_CAP_ID_VC9 else \
    "Root Complex Register Block (RCRB) Header" if cap_id == PCI_EXT_CAP_ID_RCRB else \
    "Vendor-Specific Extended Capability (VSEC)" if cap_id == PCI_EXT_CAP_ID_VNDR else \
    "Configuration Access Correlation (CAC)" if cap_id == PCI_EXT_CAP_ID_CAC else \
    "Access Control Services (ACS)"if cap_id == PCI_EXT_CAP_ID_ACS else\
    "Alternative Routing-ID Interpretation (ARI)" if cap_id == PCI_EXT_CAP_ID_ARI else \
    "Address Translation Services (ATS)" if cap_id == PCI_EXT_CAP_ID_ATS else \
    "Single Root I/O Virtualization (SR-IOV)" if cap_id == PCI_EXT_CAP_ID_SRIOV else \
    "Multi-Root I/O Virtualization (MR-IOV)" if cap_id == PCI_EXT_CAP_ID_MRIOV else \
    "Multicast" if cap_id == PCI_EXT_CAP_ID_MCAST else \
    "Page Request" if cap_id == PCI_EXT_CAP_ID_PRI else \
    "Reserved for AMD" if cap_id == PCI_EXT_CAP_ID_AMD_XXX else \
    "Resizable BAR" if cap_id == PCI_EXT_CAP_ID_REBAR else \
    "Dynamic Power Allocation (DPA)" if cap_id == PCI_EXT_CAP_ID_DPA else \
    "TLP Processing Hints (TPH)" if cap_id == PCI_EXT_CAP_ID_TPH else \
    "Latency Tolerance Reporting (LTR)" if cap_id == PCI_EXT_CAP_ID_LTR else \
    "Secondary PCI Express" if cap_id == PCI_EXT_CAP_ID_SECPCI else \
    "Protocol Multiplexing (PMUX)" if cap_id == PCI_EXT_CAP_ID_PMUX else \
    "Process Address Space ID (PASID)" if cap_id == PCI_EXT_CAP_ID_PASID else \
    "LN Requester (LNR)" if cap_id == PCI_EXT_CAP_ID_LNR else \
    "Downstream Port Containment (DPC)" if cap_id == PCI_EXT_CAP_ID_L1PMS else \
    "L1 PM Substates" if cap_id == PCI_EXT_CAP_ID_L1PMS else \
    "Precision Time Measurement (PTM)" if cap_id == PCI_EXT_CAP_ID_PTM else \
    "PCI Express over M-PHY (M-PCIe)" if cap_id == PCI_EXT_CAP_ID_MPHY else \
    "FRS Queueing" if cap_id == PCI_EXT_CAP_ID_FRSQ else \
    "Readiness Time Reporting"if cap_id == PCI_EXT_CAP_ID_RTR else \
    "Unknown"
