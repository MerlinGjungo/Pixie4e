...
/* -----------------------------------------------
    $$$nu$$$ run-time registers information
   ----------------------------------------------- */
/* Run-time registers information array */
static const WDC_REG g$$$nu$$$_Regs[] = {
    { $$$nu$$$_SPACE, $$$nu$$$_DSCR_OFFSET, WDC_SIZE_32, WDC_READ_WRITE, "DSCR",
        "Device Control Status Register" },
    { $$$nu$$$_SPACE, $$$nu$$$_DDMACR_OFFSET, WDC_SIZE_32, WDC_READ_WRITE,
        "DDMACR", "Device DMA Control Status Register" },
    { $$$nu$$$_SPACE, $$$nu$$$_WDMATLPA_OFFSET, WDC_SIZE_32, WDC_READ_WRITE,
        "WDMATLPA", "Write DMA TLP Address" },
    { $$$nu$$$_SPACE, $$$nu$$$_WDMATLPS_OFFSET, WDC_SIZE_32, WDC_READ_WRITE,
        "WDMATLPS", "Write DMA TLP Size" },
    { $$$nu$$$_SPACE, $$$nu$$$_WDMATLPC_OFFSET, WDC_SIZE_32, WDC_READ_WRITE,
        "WDMATLPC", "Write DMA TLP Count" },
    { $$$nu$$$_SPACE, $$$nu$$$_WDMATLPP_OFFSET, WDC_SIZE_32, WDC_READ_WRITE,
        "WDMATLPP", "Write DMA Data Pattern" },
    { $$$nu$$$_SPACE, $$$nu$$$_RDMATLPP_OFFSET, WDC_SIZE_32, WDC_READ_WRITE,
        "RDMATLPP", "Read DMA Expected Data Pattern" },
    { $$$nu$$$_SPACE, $$$nu$$$_RDMATLPA_OFFSET, WDC_SIZE_32, WDC_READ_WRITE,
        "RDMATLPA", "Read DMA TLP Address" },
    { $$$nu$$$_SPACE, $$$nu$$$_RDMATLPS_OFFSET, WDC_SIZE_32, WDC_READ_WRITE,
        "RDMATLPS", "Read DMA TLP Size" },
    { $$$nu$$$_SPACE, $$$nu$$$_RDMATLPC_OFFSET, WDC_SIZE_32, WDC_READ_WRITE,
        "RDMATLPC", "Read DMA TLP Count" },
    { $$$nu$$$_SPACE, $$$nu$$$_WDMAPERF_OFFSET, WDC_SIZE_32, WDC_READ, "WDMAPERF",
        "Write DMA Performance" },
    { $$$nu$$$_SPACE, $$$nu$$$_RDMAPERF_OFFSET, WDC_SIZE_32, WDC_READ, "RDMAPERF",
        "Read DMA Performance" },
    { $$$nu$$$_SPACE, $$$nu$$$_RDMASTAT_OFFSET, WDC_SIZE_32, WDC_READ, "RDMASTAT",
        "Read DMA Status" },
    { $$$nu$$$_SPACE, $$$nu$$$_NRDCOMP_OFFSET, WDC_SIZE_32, WDC_READ, "NRDCOMP",
        "Number of Read Completion w/ Data" },
    { $$$nu$$$_SPACE, $$$nu$$$_RCOMPDSIZE_OFFSET, WDC_SIZE_32, WDC_READ,
        "RCOMPDSIZE", "Read Completion Data Size" },
    { $$$nu$$$_SPACE, $$$nu$$$_DLWSTAT_OFFSET, WDC_SIZE_32, WDC_READ, "DLWSTAT",
        "Device Link Width Status" },
    { $$$nu$$$_SPACE, $$$nu$$$_DLTRSSTAT_OFFSET, WDC_SIZE_32, WDC_READ,
        "DLTRSSTAT", "Device Link Transaction Size Status" },
    { $$$nu$$$_SPACE, $$$nu$$$_DMISCCONT_OFFSET, WDC_SIZE_32, WDC_READ_WRITE,
...
#define $$$nu$$$_REGS_NUM (sizeof(g$$$nu$$$_Regs)/sizeof(g$$$nu$$$_Regs[0]))
...
static void MenuReadWriteRegs(WDC_DEVICE_HANDLE hDev)
{
    ...
    do {
        /* Display predefined registers information */
        printf("\n");
        printf("$$$nu$$$ run-time registers:\n");
        printf("--------------------------\n");
        WDC_DIAG_RegsInfoPrint(hDev, g$$$nu$$$_Regs, $$$nu$$$_REGS_NUM,
            WDC_DIAG_REG_PRINT_ALL, FALSE);

        printf("\n");
        printf("Read/write the $$$nu$$$ run-time registers\n");
        printf("-----------------------------------------\n");
        ...
        switch (option)
        {
            ...
            case MENU_RW_REGS_READ_ALL:
                WDC_DIAG_ReadRegsAll(hDev, g$$$nu$$$_Regs, $$$nu$$$_REGS_NUM, FALSE,
                    FALSE);
                break;
            case MENU_RW_REGS_READ_REG:  /* Read from a register */
                WDC_DIAG_ReadWriteReg(hDev, g$$$nu$$$_Regs, $$$nu$$$_REGS_NUM,
                    WDC_READ, FALSE);
                break;
    ...
    DWORD dwNumItems, UINT32 u32Pattern, BOOL fIsRead)
{
    ...
    else /* Verify a device-to-host (write) DMA transfer */
    {
        ...
        for (i = 0; i < dwNumItems; i++)
        {
            ...
            {
                $$$nu$$$_ERR("Device-to-host (write) DMA data mismatch: host data "
...
static void DmaOpen(WDC_DEVICE_HANDLE hDev, PDMA_STRUCT pDma, BOOL fPolling)
{
    ...
    fEnable64bit = FALSE;
    bTrafficClass = 0;
    $$$nu$$$_DmaDevicePrepare(pDma->hDma, fIsRead, wTLPNumItems, wNumTLPs,
        u32Pattern, fEnable64bit, bTrafficClass);

    if (!fPolling) /* Interrupt-based DMA */
    {
        /* Enable DMA interrupts */
        $$$nu$$$_DmaIntEnable(hDev, fIsRead);

        if (!$$$nu$$$_IntIsEnabled(hDev))
        {
            dwStatus = $$$nu$$$_IntEnable(hDev, DiagDmaIntHandler);
    ...
    else /* Polling-based DMA */
    {
        ...
        $$$nu$$$_DmaIntDisable(hDev, fIsRead);
    }

    /* Start DMA transfer */
    printf("Start DMA transfer\n");
    $$$nu$$$_DmaStart(pDma->hDma, fIsRead);

    /* Poll for DMA completion (when using polling-based DMA) */
    if (fPolling)
    {
        if ($$$nu$$$_DmaPollCompletion(pDma->hDma, fIsRead))
        {
